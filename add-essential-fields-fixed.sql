...
-- Fix the oversimplification (FIXED VERSION)

-- 1. First, check what columns exist and add the missing ones
ALTER TABLE assessment_submissions ADD COLUMN IF NOT EXISTS submitted_at TIMESTAMP WITH TIME ZONE DEFAULT NOW();
...
-- Update existing records to have proper timestamps (only if they exist and are null)
UPDATE assessment_submissions 
SET 
  submitted_at = COALESCE(submitted_at, NOW()),
  started_at = COALESCE(started_at, NOW()),
  completed_at = COALESCE(completed_at, NOW())
WHERE submitted_at IS NULL OR started_at IS NULL OR completed_at IS NULL;
