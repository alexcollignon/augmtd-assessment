...
import { Eye, EyeOff, AlertCircle, ArrowLeft, LogIn, Shield, Users } from 'lucide-react'
...
export function UnifiedLoginPage() {
  ...
  const { login: adminLogin, isLoading: adminLoading } = useAuth()
  const { login: superadminLogin, isLoading: superadminLoading } = useSuperadmin()
  
  const isLoading = adminLoading || superadminLoading
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    let success = await adminLogin(email, password)
    if (success) {
      console.log('Admin login successful, redirecting to /admin')
      window.location.href = '/admin'
      return
    }
    console.log('Admin login failed, trying superadmin login')

    // If admin login fails, try superadmin login
    success = await superadminLogin(email, password)
    if (success) {
      console.log('Superadmin login successful, redirecting to /superadmin')
  ...
  const handleDemoLogin = (demoType: 'admin' | 'superadmin') => {
    if (demoType === 'admin') {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-indigo-900 flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-lg space-y-8 relative z-10">
        ...
        <div className="text-center">
          ...
          <p className="text-blue-200 text-lg">Dashboard Access</p>
        </div>
        ...
        <div className="flex gap-4">
          <button
            ...
            className="flex-1 bg-blue-500/20 hover:bg-blue-500/30 text-white px-6 py-4 rounded-2xl font-semibold transition-all duration-200 hover:scale-105 border border-blue-400/30 backdrop-blur-sm flex items-center justify-center"
            disabled={isLoading}
          >
          ...
          <button
            ...
            className="flex-1 bg-red-500/20 hover:bg-red-500/30 text-white px-6 py-4 rounded-2xl font-semibold transition-all duration-200 hover:scale-105 border border-red-400/30 backdrop-blur-sm flex items-center justify-center"
            disabled={isLoading}
          >
